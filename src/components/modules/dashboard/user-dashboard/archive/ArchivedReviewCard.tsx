...
export default function ArchivedReviewCard({ review }: { review: IReview }) {
  ...
  const triggerDeleteConfirmation = () => {
    toast.error("Permanently Purge Record?", {
      ...
      classNames: {
        toast:
          "bg-rose-950 border border-rose-500/30 text-foreground rounded-2xl p-5",
        ...
        cancelButton:
          "text-muted-foreground hover:text-foreground font-bold text-[11px] px-4 py-2",
      },
    });
  };
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -5 }}
      className={cn(
        "group relative h-full flex flex-col overflow-hidden rounded-[2.5rem] bg-surface",
    ...
    >
          ...
          <div className="flex gap-4 w-full">
            ...
            <div className="space-y-1.5 flex-1 min-w-0">
              <div className="flex items-center justify-between w-full">
                ...
                <div className="flex items-center gap-1 px-2 py-0.5 rounded-md bg-foreground/5 border border-border">
              ...
              <h3 className="text-base font-black text-muted-foreground tracking-tight line-clamp-1 group-hover:text-foreground transition-colors duration-300">
                {review.media?.title}
              </h3>
            </div>
          </div>
            ...
            {review.tags.map((tag) => (
              <span
                ...
                className={cn(
                  ...
                  "text-[9px] font-bold text-muted-foreground/60 uppercase tracking-tighter",
                )}
              >
      ...
      {/* 📊 Action Footer */}
      <div className="p-8 pt-6 border-t border-border/50 relative z-10 flex items-center justify-between bg-surface mt-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className={cn(
              ...
              "hover:text-foreground hover:bg-foreground/10 hover:border-border",
            )}
          >
