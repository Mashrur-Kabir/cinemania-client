          ...
          <AnimatePresence>
            {isViewOpen && (
              <div className="fixed inset-0 z-[999] flex items-center justify-center p-6 md:p-10 pointer-events-auto">
                ...
                >
                  <div className="p-8 border-b border-border/50 flex items-center justify-between bg-rose-500/[0.02]">
                  ...
                  <div className="p-8 space-y-8 overflow-y-auto custom-scrollbar flex-1">
                    ...
                    <div className="p-6 rounded-3xl bg-rose-500/5 border border-rose-500/20 relative overflow-hidden">
                      ...
                      <p className="text-sm text-rose-300 italic font-medium leading-relaxed">
                    ...
                    <div className="space-y-3">
                      ...
                      <p className="text-sm leading-relaxed text-muted-foreground italic bg-foreground/[0.01] border-border/50 p-6 rounded-3xl border break-words whitespace-pre-wrap">
                        ❝{review.content}❞
                      </p>
                    </div>
                    ...
                    {review.tags && review.tags.length > 0 && (
                      <div className="flex flex-wrap gap-2 pt-2">
                        {review.tags.map((tag) => (
                          <span
                            key={tag}
                            className="px-3 py-1 rounded-full bg-foreground/[0.02] border border-border/50 text-muted-foreground text-[9px] font-bold uppercase tracking-tighter"
                          >
                            #{tag}
                          </span>
                        ))}
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="flex items-center gap-3 max-w-[200px]">
          <div className="relative h-10 w-7 rounded overflow-hidden border border-border/50 shrink-0 opacity-60">
            {media?.posterUrl ? (
              <Image
                src={media.posterUrl}
                alt="Poster"
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="flex items-center gap-2 opacity-60">
          <div className="p-1.5 rounded-full bg-foreground/5 border border-border/50 text-muted-foreground shrink-0">
