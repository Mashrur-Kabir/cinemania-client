...
export default function NotificationCenter({
  ...
  /**
   * 🎯 THE FIX:
   * We explicitly define the generic types <TData, TError, TVariables>.
   * This tells markRead that it MUST accept a 'string' (the notification ID).
   */
          ...
          >
            <Bell
              className={cn(
                ...
                unreadCount > 0
                  ? "text-primary" // 🎯 Removed scale-110 here to avoid double-scaling
              ...
              {notifications.map((n) => {
                ...
                return (
                  <Link
                    ...
                    className={cn(
                      "flex items-start gap-4 p-5 transition-all hover:bg-white/[0.03] relative group",
                      !n.isRead && "bg-primary/[0.02]",
                    )}
                  >
                    <div
                      className={cn(
                        "mt-1 flex-shrink-0 size-8 rounded-xl flex items-center justify-center bg-white/[0.03] border border-white/5 group-hover:border-white/10 transition-colors",
                        config.color,
                      )}
                    >
