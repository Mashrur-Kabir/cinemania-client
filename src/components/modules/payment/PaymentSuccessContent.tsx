...
export default function PaymentSuccessContent({
  ...
  return (
    <div className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden bg-surface text-foreground p-6">
      ...
      >
        {/* 🏆 Premium Glass Panel */}
        <div className="rounded-[3rem] bg-surface/90 border border-border p-8 sm:p-12 shadow-2xl relative overflow-hidden">
          ...
          >
            ...
            <h2 className="text-4xl sm:text-5xl font-black uppercase italic tracking-tighter text-foreground">
              Access <span className="text-muted-foreground">Granted.</span>
            </h2>

            <p className="text-sm text-muted-foreground font-medium leading-relaxed">
              Your cinematic journey is now unrestricted. Welcome to the{" "}
              {isLoading ? (
                <span className="inline-block w-12 h-4 bg-surface/20 animate-pulse rounded align-middle" />
              ) : (
                <span
                  className={cn(
          ...
          {sessionId && (
            ...
            >
              <div className="flex items-center justify-between gap-3 p-4 rounded-2xl bg-foreground/[0.04] border border-border relative group">
                ...
                <button
                  onClick={handleCopy}
                  className="absolute right-4 p-2 rounded-lg bg-foreground/5 hover:bg-foreground/10 border border-border transition-all text-muted-foreground hover:text-foreground"
          ...
          <motion.div variants={itemVariants} className="flex flex-col gap-3">
            <Link
              href="/dashboard"
              className="group relative w-full flex items-center justify-center gap-3 px-8 py-4 rounded-full bg-primary text-primary-foreground font-black uppercase text-[11px] tracking-widest overflow-hidden transition-all duration-300 hover:bg-primary/90 active:scale-95 shadow-sm transform-gpu [backface-visibility:hidden]"
            >
            ...
            <Link
              href="/discovery"
              className="group w-full flex items-center justify-center gap-3 px-8 py-4 rounded-full bg-surface/90 border border-border text-foreground font-black uppercase text-[11px] tracking-widest transition-all duration-300 hover:bg-surface/95 active:scale-95 transform-gpu [backface-visibility:hidden]"
            >
