...
const MultiSelectFilterControl = ({
  ...
  return (
    <div className="space-y-4">
      {/* 🎯 THE FIX: custom-scrollbar applied here */}
      <div className="max-h-52 space-y-1 overflow-y-auto pr-2 custom-scrollbar">
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-3">
        {RANGE_OPERATORS.map((operator) => (
          <div key={operator} className="space-y-1.5">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {onClearAll && totalActiveFilters > 0 && (
          <Button
            type="button"
            variant="ghost"
            className="h-9 text-rose-400 hover:bg-rose-500/10 hover:text-rose-500 transition-colors"
            onClick={onClearAll}
            disabled={isLoading}
          >
        ...
        <div className="flex flex-wrap gap-2 pt-1">
          {activeBadges.map((badge) => (
            <Badge
              key={badge.key}
              variant="outline"
              className="flex items-center gap-1.5 pr-1.5 py-1 text-[10px] font-bold uppercase tracking-wider bg-primary/10 text-primary border-primary/30 hover:bg-primary/20 transition-colors"
            >
              {badge.label}
              <button
                type="button"
